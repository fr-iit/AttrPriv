from RecSys_Utils import confusion_matrix, classification_accuracy, one_hot, plot_line, performance_measures, ROC_cv, ROC_multiclass
...
def ada_classifier(X, T, multiclass = False):
...
def rf_classifier(X, T, multiclass=False):
    X_train, T_train = X[0:int(0.9 * len(X))], T[0:int(0.9 * len(X))]
    X_test, T_test = X[int(0.9 * len(X)):], T[int(0.9 * len(X)):]
...
def xgb_classifier(X, T, multiclass = False):

    X_train, T_train = X[0:int(0.9 * len(X))], T[0:int(0.9 * len(X))]
    X_test, T_test = X[int(0.9 * len(X)):], T[int(0.9 * len(X)):]

    classifier = xgb.XGBClassifier(objective='multi:softmax', num_class=2, random_state=42)
    classifier.fit(X_train, T_train)
